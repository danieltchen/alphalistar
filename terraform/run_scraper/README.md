...
- Two **Secrets Manager** secrets:
  - **RDS credentials secret** (`username`/`password` — aliases `user` also supported) referenced as `AWS_SECRET_NAME` in the Lambdas.
  - **App runtime secret** referenced as `AWS_APP_SECRET_NAME` in the Lambdas. At minimum it should include DB connection fields (`dbname` or `dbInstanceIdentifier`, `host`, `port`) and `OPENAI_API_KEY`.
...
2. **Log in to ECR** and **push** the image. Repository URL is in `terraform output worker_ecr_repository_url` (or construct `...amazonaws.com/<name_prefix>-scrape-worker` in your account/region).
  ```bash
   ...
   docker buildx build --platform linux/amd64 --provenance=false   --sbom=false -f run_scraper/Dockerfile.lambda-worker    -t $(terraform output -raw worker_ecr_repository_url):<tag> --push .
  ```
...
`scraper/scrape.py` calls `DatabaseConnector.get_db_config()` **with no arguments**. With this module, DB credentials come from the **RDS credentials secret** (`AWS_SECRET_NAME`), while the non-credential connection fields (`DB_NAME`, `DB_HOST`, `DB_PORT`) come from either:

- `worker_environment_extra` in `terraform.tfvars`, and/or
- the **app runtime secret** (`AWS_APP_SECRET_NAME`) JSON keys (`dbname`/`dbInstanceIdentifier`, `host`, `port`).

For press-release scraping, `OPENAI_API_KEY` is loaded from `OPENAI_API_KEY` in the environment first, and then from the app runtime secret (`AWS_APP_SECRET_NAME`) if it is not set.
...
| Variable                                | Purpose                                                                                                             |
| --------------------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| `dispatcher_schedule_expression`        | EventBridge cron or `rate(...)`; default is weekdays at 02:00 UTC                                                   |
